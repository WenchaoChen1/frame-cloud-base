...
public class HttpCryptoProcessor extends AbstractStampManager<String, SecretKey> {
    ...
    public String encrypt(String identity, String content) throws SessionInvalidException {
        try {
            ...
            log.debug("[GstDev Cloud] |- Encrypt content from [{}] to [{}].", content, result);
            return result;
        } catch (StampHasExpiredException e) {
            log.warn("[GstDev Cloud] |- Session has expired, need recreate.");
            throw new SessionInvalidException();
        } catch (Exception e) {
            log.warn("[GstDev Cloud] |- Symmetric can not Encrypt content [{}], Skip!", content);
            return content;
        }
    }
    ...
    public String decrypt(String identity, String content) throws SessionInvalidException {
        try {
            ...
            log.debug("[GstDev Cloud] |- Decrypt content from [{}] to [{}].", content, result);
            return result;
        } catch (StampHasExpiredException e) {
            log.warn("[GstDev Cloud] |- Session has expired, need recreate.");
            throw new SessionInvalidException();
        } catch (Exception e) {
            log.warn("[GstDev Cloud] |- Symmetric can not Decrypt content [{}], Skip!", content);
            return content;
        }
    }
    ...
    public SecretKey nextStamp(String key) {
        ...
        log.debug("[GstDev Cloud] |- Generate secret key, value is : [{}]", secretKey);
        return secretKey;
    }
    ...
    private SecretKey getSecretKey(String identity) throws StampHasExpiredException {
        if (isSessionValid(identity)) {
            ...
            if (ObjectUtils.isNotEmpty(secretKey)) {
                log.trace("[GstDev Cloud] |- Decrypt Or Encrypt content use param identity [{}], cached identity is [{}].", identity, secretKey.getIdentity());
                return secretKey;
            }
        }
    ...
    private String decryptFrontendPublicKey(String content, String privateKey) {
        ...
        log.debug("[GstDev Cloud] |- Decrypt frontend public key, value is : [{}]", frontendPublicKey);
    ...
    private String encryptBackendKey(String symmetricKey, String publicKey) {
        ...
        log.debug("[GstDev Cloud] |- Encrypt symmetric key use frontend public key, value is : [{}]", encryptedAesKey);
